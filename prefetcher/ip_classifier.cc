...
void Ip_classifier::update_when_ip_is_full() {
    ...
    this->ip_last_cl.erase(victim->first);
}
...
void Ip_classifier::update(uint64_t ip, uint64_t addr) {
    ...
    //new
    if (this->ip_last_cl.find(ip) == this->ip_last_cl.end()){
        if (this->ip_jump.size() == IP_NUM){
            this->update_when_ip_is_full();
        }
        this->ip_last_cl[ip] = cacheline;
        this->ip_jump[ip] = make_pair(0, 1);
        return;
    }
    else{
        //
        uint64_t last_cacheline = this -> ip_last_cl[ip];
        uint64_t last_page = this->ip_last_cl[ip] >> (LOG2_PAGE_SIZE - LOG2_BLOCK_SIZE);
        if(last_cacheline == cacheline) {
            return;
        }
