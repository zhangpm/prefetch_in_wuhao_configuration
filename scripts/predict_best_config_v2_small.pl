...
my @config_array = ("hawkeye_dynamic_oracle_config0", "hawkeye_dynamic_oracle_config40", "hawkeye_dynamic_oracle_config80");
...
#    print "$dut_file \n";
    ...
    for (my $i=0; $i < 4; $i++) 
    {
        ...
        my $core_dram_latency = $dram_latency;   
        my $core_hitrate = compute_hits($dut_file, $i);
        my $core_baseline_hitrate = compute_hits($hawkeye_baseline_file, $i);
        #$percore_predicted_improvement[$i] = (20*$core_baseline_hitrate + $dram_latency*(1-$core_baseline_hitrate))/(20*$core_hitrate + ($dram_latency+2*($dut_used_bw - $baseline_used_bw)*abs($dut_used_bw - $baseline_used_bw))*(1-$core_hitrate));
       ...
       #$percore_predicted_improvement[$i] = (20*$core_baseline_hitrate + $dram_latency*(1-$core_baseline_hitrate))/(20*$core_hitrate + ($dut_dram_latency)*(1-$core_hitrate));
    ...
    #$b = (20*$core_hitrate + ($dram_latency+2*($dut_used_bw - $baseline_used_bw)*abs($dut_used_bw - $baseline_used_bw))*(1-$core_hitrate));
        print "      $core_hitrate $core_baseline_hitrate $core_dram_latency $percore_predicted_improvement[$i]\n";
    }
    ...
    print "$counter: $predicted_improvement \t$dut_used_bw \t$dut_dram_latency \t$aggressive_latency \t$config_hitrate[$counter] -- \t$config_speedup[$counter]\n";
    $counter++;
}
